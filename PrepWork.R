...
#install.packages(c("tidyverse", "dplyr", "httr", "jsonlite", "stringr"))
...
# Helper set of vectors for containing valid icaoIDs.  Had to break this up into four batch of 504 icaoIDs because of the limitations of characters per row in R.  
...
metar_all <- build_url(endpoint = "metar", hours = 12)
metar_all <- metar_all |>
...
metar_summ <- metar_all |>
  summarize(
    ...
    min_wspd = min(wspd, na.rm = TRUE),
    max_wspd = max(wspd, na.rm = TRUE),

    mean_visib = mean(visib, na.rm = TRUE),
    median_visib = median(visib, na.rm = TRUE),
    sd_visib = sd(visib, na.rm = TRUE),
    min_visib = min(visib, na.rm = TRUE),
